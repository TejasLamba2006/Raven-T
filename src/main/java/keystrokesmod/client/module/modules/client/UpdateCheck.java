        ...
        task = () -> {
            ...
            if (latest.isNewerThan(current)) {
                Utils.Player.sendMessageToSelf(
                        "The current version or Raven T is outdated. Visit https://github.com/TejasLamba2006/Raven-T/releases/latest to download the latest version.");
                Utils.Player.sendMessageToSelf("https://github.com/TejasLamba2006/Raven-T/releases/latest");
            }

            if (current.isNewerThan(latest)) {
                Utils.Player.sendMessageToSelf("You are on a beta build of raven");
                Utils.Player.sendMessageToSelf("https://github.com/TejasLamba2006/Raven-T/");
            } else {
            ...
            if (openLink.isToggled()) {
                try {
                    ...
                    Utils.Player
                            .sendMessageToSelf("&cFailed to open page! Please report this bug in Raven T's discord");
                }
            }
